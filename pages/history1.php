...
if($rs1->num_rows > 0){
  ...
  if($settings['logs']=="true"){
    ...
    $sql = "insert into logs(lid,macaddress,devicename,cid,message,datetime)values(DEFAULT,'$IP','$mycomputername','$sid','Clicked Practise Test Results','$timestamp')";
    $conn->query($sql);
  }
      ...
      <div class="content">
        <div class="row">
          <div class="col-md-12 col-xs-12 col-lg-12">
            <div class="card">
              <div class="card-body">
                <center><h2 class="text">Practise Exam</h2></center>
                <hr>
                <div class="table-responsive" style="overflow-y:hidden;padding-left:20px;">
                  <?php 
                    $cid = $sid;
                    $sql = "SELECT * FROM results where cid='$cid' and examtype='practisetest' group by(examname) order by rid desc";
                      ...
                      <tbody>
                        <?php
                            ...
                            while($row = $result->fetch_assoc()) {
                              $examno = $examno + 1;
                                echo "<tr><td>" . $examno . "</td>"."<td>" . $row['examname'] . "</td><td><a href='testlist.php?en=".md5($row['examname'])."&type=".md5('practisetest')."' class='btn btn-info'>List Attempts</a></td></tr>";
