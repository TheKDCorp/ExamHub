 ...
 <?php
  ...
  $type = htmlspecialchars(stripslashes(trim(mysqli_real_escape_string($conn,$_GET['type']))));
  $en = htmlspecialchars(stripslashes(trim(mysqli_real_escape_string($conn,$_GET['en']))));

  $mycomputername = ""; 
  $IP = "";
?>
<div style="display:none;overflow-y:hidden;">
<?php 
  $sql = "SELECT * from testcontinue where examname='$en' and examtype='$type'";
  $resultmine = $conn->query($sql);
  if($resultmine->num_rows > 0){
    while($rowmine = $resultmine->fetch_assoc()){
      $cid=$rowmine['studentid'];
      $qindex=$rowmine['qindex'];
      $timeleft=$rowmine['continuetime'];
      $examname = $en;
      $examtype=$type;

      $sql3 = "select * from students where sid='$cid'";
      ...
      $sql = "select * from questionpaper where name='$en' and examtype='$type'";
      $result=$conn->query($sql);
      if($result->num_rows > 0){
        $row3=$result->fetch_assoc();
        $totaltimeinmins = $row3['time'];
      }else{
        $eid = "";
        $examname = "";
        $type = "";
      ...
      $timetaken = $totaltimeinmins - $timeleft . ":00";
      $timeleft = $timeleft . ":00";

      $sql3    = "select * from answers_new where cid='$cid' and qindex='$qindex' and examname='$en' and examtype='$type'";
      $result3 = $conn->query($sql3);
      if($result3->num_rows > 0){
          while($row3 = $result3->fetch_assoc()){
              
              $qid=$row3['qid'];
              $ai = $row3['choosedoption'];
              $part = $row3['part'];
      ...
      $sql33  = "delete from testcontinue where examname='$examname' and studentid='$cid' and examtype='$type' and qindex='$qindex'";
      $conn->query($sql33);  
      ...
      $sql    = "select * from answers where cid='$cid' and qindex='$qindex' and examname='$examname' and examtype='$type'";
      $result = $conn->query($sql);
      if($result->num_rows > 0){
...
<script type="text/javascript">
  $(document).ready(function() {
    $("#mytitle").text("Exam Result Generate");
});
</script>
      ...
      <div class="content">
        <div class="row">
          <div class="col-md-12 col-xs-12 col-lg-12">
            <div class="card">
              <div class="card-body">
                <h3>Exam Result Generated: <span style="color:green">Done!</span></h3>
                <hr>
                <center><a href="./results.php" class="btn btn-primary">Exit</a></center>
              </div>
            </div>
          </div>
        </div>
